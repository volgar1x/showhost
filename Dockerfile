...
RUN mkdir -p /showhost-backend /showhost-backend/libs/transport /showhost-backend/projects/backend
WORKDIR /showhost-backend
...
RUN sbt backend/graalvm-native-image:packageBin

# ffmpeg {{{
FROM ghcr.io/linuxserver/baseimage-ubuntu:arm64v8-jammy-version-c3aef6a6 as ffmpeg
ARG FFMPEG_VERSION
...
# yt-dlp {{{
FROM debian:12-slim AS ytdlp
RUN \
    apt update && \
    apt install -y curl && \
    curl -L https://github.com/yt-dlp/yt-dlp/releases/download/2023.10.13/yt-dlp_linux_aarch64 -o /bin/yt-dlp && \
    chmod +x /bin/yt-dlp
# }}}
...
RUN \
  ...
  rm -rf \
    ...
    /var/tmp/* && \
  ls /usr/local/lib/ && \
  mv /usr/local/lib/lib*.so* /lib/$(uname -m)-linux-gnu/
COPY --from=ytdlp /bin/yt-dlp /bin/yt-dlp
COPY --from=build /showhost-backend/projects/backend/target/graalvm-native-image/showhost-backend /bin/showhost-backend
CMD ["/bin/showhost-backend"]
